...
function submitAnswers() {
  ...
  questions.forEach((q, index) => {
    const selectedAnswer = document.querySelector(`input[name="question${index}"]:checked`);
    const correctAnswer = document.querySelector(`input[name="question${index}"][value="${q.correctAnswer}"]`);
    const answerContainer = selectedAnswer.parentNode;
    const correctContainer = correctAnswer.parentNode;
    if (selectedAnswer && parseInt(selectedAnswer.value) === q.correctAnswer) {
      score++;
    ...
    questions.forEach((q, index) => {
      ...
      timers[index] = setInterval(() => {
        ...
        if (time <= 0) {
          ...
          // Verifica se o alert já foi exibido
          if (!alertExibido) {
            alert("Tempo para respostas esgotado! A página será recarregada.");
            alertExibido = true;
