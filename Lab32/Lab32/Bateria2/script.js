const perguntas = [
  ...
  timerInterval = setInterval(() => {
    tempoRestante--;
    document.getElementById("tempo").innerHTML = `Tempo restante: ${tempoRestante} segundos`;
    if (tempoRestante <= 0) {
      verificarResposta(null); // forçar verificarResposta quando o tempo acaba
    }
  }, 1000);
  ...
  function verificarResposta(event) {
    if (event) event.preventDefault(); // evitar que o formulário seja submetido
    ...
    if (respostaSelecionada !== null) {
      ...
      if (perguntas[indicePergunta].opcoes[indiceResposta] === perguntas[indicePergunta].opcoes[perguntas[indicePergunta].resposta]) {
        acertos++; // incrementar contador de acertos
        pontuacao++;
        document.getElementById("resultado").innerHTML = `Você acertou! Pontuação: ${pontuacao} (${acertos} acertos)`;
        document.getElementById("resultado").classList.add("correta");
      } else {
        document.getElementById("resultado").innerHTML = `Você errou! A resposta correta era ${perguntas[indicePergunta].opcoes[perguntas[indicePergunta].resposta]}. Pontuação: ${pontuacao} (${acertos} acertos)`;
      ...
      if (indicePergunta < perguntas.length) {
        ...
        document.getElementById("opcao3").nextElementSibling.innerHTML = perguntas[indicePergunta].opcoes[2];
        document.getElementById("opcao4").nextElementSibling.innerHTML = perguntas[indicePergunta].opcoes[3];
        tempoRestante = 30; // resetar tempo
        document.getElementById("tempo").innerHTML = `Tempo restante: ${tempoRestante} segundos`;
        clearInterval(timerInterval); // parar timer
        timerInterval = setInterval(() => {
          tempoRestante--;
          document.getElementById("tempo").innerHTML = `Tempo restante: ${tempoRestante} segundos`;
          if (tempoRestante <= 0) {
        ...
        document.getElementById("resultado").innerHTML = `Fim do quiz! Pontuação final: ${pontuacao} (${acertos} acertos)`;
        document.getElementById("resultado").classList.add("animacao-final");
        clearInterval(timerInterval); // parar timer
      }
    } else {
